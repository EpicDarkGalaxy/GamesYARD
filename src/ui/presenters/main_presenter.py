...
class MainPresenter:
    ...
    def bind_signals(self):
        ...
        self.app_core.asset_manager.thumb_ready.connect(self._handle_thumb_result)
        _ = self.app_core.search_manager.search_state_changed.connect(
            self.view.update_search_button
        )

        # Main View
        self.view.signals.close.connect(self._on_close) # I want MainPresenter to decide weather to accept or ignore the event

        # Main View (Search Bar)
        self.view.main_ui.btn_search_2.clicked.connect(self.request_search)

        # Main View (Side bar nav)
        self.view.main_ui.btn_search.clicked.connect(self.view.toggle_search_bar)
    ...
    def request_search(self):
        ...
        self.app_core.search_manager.perform_search(query=query)
    ...
    def request_thumb(self, card_id: str):
        ...
        if card:
            ...
            self.app_core.asset_manager.get_thumbnail(card_id, img_url)
        else:
